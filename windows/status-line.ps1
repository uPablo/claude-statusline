...
if ($null -ne $cost) { $p2 += "$C_COST`$$('{0:0.00}' -f [double]$cost)$R" }
...
$p3 = @()
if ($null -ne $ctx_pct) { $p3 += (Seg 'ctx' $C_CTX $ctx_pct $ctx_suffix) }
...
if ($null -ne $r5) { $p3 += (Seg '5h' $C_5H $r5 (Reset-Label (Get-P $j @('rate_limits','five_hour','resets_at')))) }
if ($null -ne $r7) { $p3 += (Seg 'wk' $C_WK $r7 (Reset-Label (Get-P $j @('rate_limits','seven_day','resets_at')))) }
if ($null -ne $ro) { $p3 += (Seg 'opus' $C_OPUS $ro (Reset-Label (Get-P $j @('rate_limits','seven_day_opus','resets_at')))) }
if ($null -ne $rs) { $p3 += (Seg 'sonnet' $C_SONNET $rs (Reset-Label (Get-P $j @('rate_limits','seven_day_sonnet','resets_at')))) }
$L3 = ($p3 -join $ISEP)
